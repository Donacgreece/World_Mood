...
create index if not exists mood_entries_geo_idx on public.mood_entries (lat_bucket, lng_bucket) where lat_bucket is not null and lng_bucket is not null;
...
with check (
  ...
  and char_length(coalesce(note,'')) <= 160
);

-- Optional cleanup job. Run manually or schedule with pg_cron if enabled.
